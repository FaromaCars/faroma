...
export default function NewCars() {
  ...
  const [forceUpdate, setForceUpdate] = useState(false); // triggers re-render for wishlist
  ...
  // Fetch new cars on mount
  useEffect(() => {
    const fetchCars = async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/cars?condition=NEW");
        const data = await res.json();
        setCars(data);

        // Preload first-page images
        const firstImages = data
          .slice(0, carsPerPage)
  ...
  const toggleFavorite = (car) => {
    ...
    if (favorites.find((item) => item._id === car._id)) {
      favorites = favorites.filter((item) => item._id !== car._id);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {currentCars.map((car) => (
                    <Link
                      key={car._id}
                      to={`/cars/${car._id}`}
                      className="border p-4 rounded-xl shadow hover:shadow-xl bg-white"
                    >
                      <img
                      ...
                      <div
                        ...
                        } flex items-center rounded-lg p-2 text-white justify-center font-semibold gap-2 w-full cursor-pointer mt-2`}
                      >
                        <p>
                          {isCarFavorite(car._id)
